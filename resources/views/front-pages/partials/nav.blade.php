{{-- ======================================================= --}}
{{-- == WRAPPER HEADER UTAMA (YANG DIPERBARUI) == --}}
{{-- ======================================================= --}}
<header class="fixed top-0 left-0 right-0 z-50">
    ...
    <div class="bg-primary-dark text-white text-sm hidden md:block">
        <div class="container mx-auto px-6 flex justify-between items-center py-2">
            ...
            <div class="flex items-center space-x-6">
                ...
                <a href="{{ $contact['url'] }}" class="flex items-center space-x-2 transition-opacity hover:opacity-80">
    ...
    {{-- NAVIGASI UTAMA --}}
    {{-- PERHATIKAN: kelas `fixed` sudah dipindahkan ke tag <header> di atas --}}
        <nav x-data="{ atTop: true, isMobileMenuOpen: false }" @scroll.window="atTop = (window.scrollY < 10)"
            @keydown.escape.window="isMobileMenuOpen = false"
            class="w-full transition-all duration-300 shadow-md bg-white/95 backdrop-blur-sm">
            ...
            <div x-show="isMobileMenuOpen" x-transition:enter="transition ease-out duration-200"
                ...
                class="absolute left-0 w-full bg-white shadow-md top-full md:hidden" style="display: none;">
                <div class="flex flex-col px-6 pt-2 pb-4 space-y-2">
                    ...
                    <div x-data="{ open: false }" class="text-gray-700">
                        ...
                        <div x-show="open" x-transition class="pt-2 pb-1 pl-4 space-y-2">
                            ...
                            <a href="{{ $child->url ?? '#' }}"
                                class="block py-1 text-sm text-gray-600 hover:text-purple-600">
                                {{ $child->name }}
                            </a>
                            @endforeach
                        </div>
                    ...
                    {{-- url Biasa Mobile --}}
                    <a href="{{ $menu->url ?? '#' }}"
                        class="block py-2 font-semibold text-gray-700 hover:text-purple-600">{{
                        $menu->name }}</a>
                    @endif
                    @endforeach
    ...
    {{-- ======================================================= --}}
    {{-- == BAGIAN PENTING: PEMBERI JARAK (SPACER) == --}}
    {{-- ======================================================= --}}
    <div class="h-[142px]"></div>