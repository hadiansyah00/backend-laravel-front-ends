{{--
    Section: VISI-MISI
    Generic Visi & Misi section. 
    Replaces: kebidanan/visi-misi, farmasi/visi-misi, gizi/visi-misi
    
    JSON keys: title, subtitle (optional), visi, misi (array), section (legacy key)
--}}
...
<section class="py-20 bg-slate-50" id="visi-misi">
    <div class="max-w-6xl px-6 mx-auto lg:px-12">
        ...
        <div class="mb-16 text-center">
            ...
            @else
                <p class="mt-4 text-lg leading-8 text-gray-600">Landasan dan tujuan yang menjadi pedoman kami dalam berkarya.</p>
            @endif
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 p-10 bg-white shadow-lg gap-y-12 lg:grid-cols-2 lg:gap-x-16 rounded-2xl">
            ...
            {{-- Visi --}}
            @if (!empty($data['visi'] ?? $data['vision'] ?? null))
            <div>
                <div class="flex items-center mb-4 gap-x-4">
                    <span class="flex items-center justify-center w-12 h-12 bg-indigo-100 rounded-xl">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="text-indigo-600 w-7 h-7">
                ...
                <p class="leading-relaxed text-gray-600">
                    {{ $data['visi'] ?? $data['vision'] ?? '' }}
                </p>
            </div>
            @endif

            {{-- Misi --}}
            @php $misiItems = $data['misi'] ?? $data['mission'] ?? []; @endphp
            ...
            <div>
                ...
                <ul class="space-y-4 text-gray-600">
                    ...
                    <li class="flex items-start">
                        <svg class="flex-shrink-0 w-6 h-6 mt-0.5 text-teal-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
