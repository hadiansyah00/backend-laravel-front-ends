...
export default function Form({ tag }) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{isEdit ? 'Edit Tag' : 'Tambah Tag'}</h2>}>
            ...
            <div className="max-w-2xl mx-auto py-6">
                <div className="bg-white dark:bg-gray-900 shadow-sm sm:rounded-2xl border border-gray-100 dark:border-gray-800 p-6">
                    <form onSubmit={submit} className="space-y-6">
                        <div>
                            <label className="block border-l-4 border-indigo-500 pl-2 text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Topik / Nama Tag</label>
                            <input
                                type="text"
                                value={data.name}
                                ...
                                placeholder="Cth: Beasiswa, Seminar"
                            />
                        ...
                        <div className="flex justify-end gap-3 pt-4">
                            <Link href={route('admin.tags.index')} className="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition">Batal</Link>
                            <button type="submit" disabled={processing} className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition flex items-center gap-2">
                                {processing ? <i className="fas fa-spinner fa-spin"></i> : <i className="fas fa-save"></i>} Simpan Tag
                            </button>
                        </div>
                    </form>
                </div>
